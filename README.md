...
参考药品入口会在回答滚动过程中原地采集，不再在回答完成后重复扫描。打开抽屉后先从标题拖拽区完全展开列表，确认全屏视口后才从首项开始截图，避免在药品内容区拖动导致第一帧落到中间卡片。随后不设置药品页数上限，持续滚动，只有连续滑动后稳定画面不再变化才确认到达末项；药品图片全部就绪且滚动接缝通过验证后，才关闭抽屉并验证正文滚动位置已经恢复。首屏、图片加载、列表到底、滚动接缝或正文恢复任一项未完成都会重试，最终仍失败则该题不算成功。超出单张长图高度时会分片保存，所有分片共同组成完整药品列表。
