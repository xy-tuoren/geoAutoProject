...
问题发送后先比较聊天内容区域的 ADB 原始 PNG，顶部系统状态栏和底部输入区不参与稳定性判断；内容连续 3 秒无变化且层级不再显示生成状态后进入小荷完成确认。新会话会在当前回答底部使用交替触点持续探测；若 UI 层级严格识别到聊天区下部居中的固定到底按钮，则先由 uiautomator2 直达底部。点击后会重新读取层级，按钮仍存在时最多再受控点击两次，每次都使用最新坐标；连续三次仍未消失便停止点击并交给滚动探测，识别不到按钮时也直接逐屏向下探测。完成确认和返回顶部只负责边界导航，因此使用按实时聊天视口计算的约 78% 长距离快速滑动；正式截图仍保持约 45% 高重叠滚动。两种路径都只有在底部至少连续三次不可继续滚动、画面持续约 6 秒不变且层级没有生成状态后，才允许回到顶部开始正式截图；直达按钮只减少到达底部的滚动次数，不替代完整性验证，也不依赖免责声明、操作栏、问诊卡片或固定文案。该阶段通过 `reply_completion_navigation_method`、`reply_completion_navigation_clicks`、`reply_completion_navigation_target_cleared`、`reply_completion_confirmed_before_capture`、`reply_completion_confirmation_probes`、`reply_completion_confirmation_resets` 和 `reply_completion_confirmation_quiet_ms` 记录路径与证据。完成确认后使用两个交替横向触点持续向上滚动。新会话只有在连续两次稳定画面无变化，并且 uiautomator2 层级确认完整可见的右侧问题气泡与本题文字严格一致时才确认到顶；画面不动但没有本题问题气泡会明确失败，不能把回答尾部误判为顶部。若小荷 Compose 在快速回顶后短暂只暴露外层窗口，脚本会有限重读层级；仍稀疏时只重启一次 uiautomator2 sidecar并进行最终只读确认，不点击、不输入，也不放宽问题气泡标准。到顶后对 ADB 原始 PNG 执行 OCR，识别“根据…篇资料为你总结”并按物理截图与逻辑 UI 尺寸映射点击，确认资料展开后才截取第一屏；资料展开导致页面重排后还会再次确认同一问题气泡完整位于首屏。

小荷新会话会复用完成确认已经取得的稳定底部帧，不再为同一状态额外执行一次双 PNG 夹心。返回顶部和引用资料展开后的稳定等待优先使用 scrcpy 活动标记，在画面静止后读取一次层级和一张 ADB 原始 PNG；scrcpy 无法确认、捕获期间仍有活动或层级仍在加载时，才自动回退到 ADB 夹心校验。顶部仍必须连续两次滚动无变化，回答底部仍必须通过原有三次持续探测，正式截图和接缝验证条件不变。
