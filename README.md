...
Windows 安装包内置的自动更新源为 `http://104.168.30.172/geo-updates/win/`。服务器端下载地址固定为 `question-automation-electron-setup.exe`，每次版本标签构建会在保留 GitHub Release 的带版本备份同时，仅在更新服务器保留这一份安装包、对应 `.blockmap` 与 `latest.yml`；旧安装包会在发布新清单前删除，`latest.yml` 始终最后发布，避免客户端读取到尚未完整上传的版本。GitHub 仓库需配置 `UPDATE_SERVER_HOST`、`UPDATE_SERVER_USER` 和 `UPDATE_SERVER_SSH_PASSWORD` 三个 Actions Secrets。待域名 HTTPS 证书可用后，应将该地址统一切换为 HTTPS 域名，避免使用裸 IP 的明文更新通道。
    ...
    调试产物/
      ...
      01_小荷AI医生APP/
        001_儿童腹泻脱水用什么药/
          ...
          接缝汇总.json              # 帧/接缝数量不变量、逐次滚动决策和异常证据索引
          接缝诊断/                  # 仅存在重采或失败接缝时生成
            接缝_001/
              上一帧.png/.xml
              滚动后.png/.xml
...
`交付图片/` 不写入 XML、JSON、日志或失败截图；正式图片也不会在调试区重复保存。`回答.json` 通过绝对路径关联交付图片、XML、单题日志、批次日志、性能分析和接缝汇总，并用 `artifact_layout_version=5` 标记当前结构。版本 4 新增每题 `性能分析.json`：记录 UI、ADB、OCR、截图、滚动、scrcpy 等底层操作的开始/结束时间、耗时、结果和安全参数，汇总各操作的次数、失败数、总耗时、平均值、P95、最大值、最慢操作和优化候选。版本 5 新增每题 `接缝汇总.json`；发生接缝重采或安全降级时，还会在 `接缝诊断/` 保存上一帧、滚动后帧、重采帧及对应 UI 层级和候选位移。接缝汇总在正式拼图前写入，因此即使随后因帧数和接缝数不一致而失败，仍能直接读取数量不变量和异常位置。所有入口共用同一套失败现场采集，诊断文件只进入 `调试产物/`，不会污染交付内容。

`执行日志.jsonl` 每行是一条独立 JSON 事件，包含时间、顺序号、耗时、入口、问题及事件类别。回答翻页、接缝降级、引用资料、参考药品入口发现、抽屉展开、每页采集、末项确认、图片加载状态、重试、失败恢复以及每次底层操作的完成/失败都会形成可检索事件。排查时优先读取 `batch-summary.json` 定位失败题，再读取该题 `失败现场.json` 中的 `operation_telemetry`：这里直接给出当前阶段、最后失败操作、失败前操作链和最慢操作；随后查看 `性能分析.json` 的阶段耗时、按操作聚合排行与优化建议，必要时再结合 `执行日志.jsonl`、`回答.xml` 和失败截图复现判断。异常包装会递归保留有限层级的 `cause`，头条路由重试耗尽时控制台也会直接附带最后一次底层拒绝原因。输入问题正文不会重复写入操作详情，输入操作只记录字符数。
...
所有入口在发送或搜索前都会强制清空并回读核对输入框文本。若 FastInputIME 的清空广播偶发失败、导致旧关键词与新问题拼接，脚本只在已确认回读不一致后，通过 uiautomator2 对当前聚焦输入控件执行一次原子 `setText` 替换；再次回读精确一致后才允许继续，仍不使用 ADB 输入，也不会重复提交问题。面板默认要求每题新建会话；小荷点击新会话入口后必须确认中央消息区已经没有旧会话内容，并且新会话输入页连续两次稳定，才会记录 `new_session_performed=true`。空白新会话页在聚焦前可能只暴露“输入问题 或 按住说话”的文字入口而没有 `EditText`，该状态会按输入入口和空白消息区共同确认；任务开始时若已经处于这个干净页面则不再重复点击。点击后若新会话入口在转场中暂时消失，会继续等待干净输入页而不盲目重点击；只有只读层级明确证明旧会话完整保留且入口重新出现时，才从最新坐标受控重试，最多三次。三次仍未切换则在输入前停止本题，不能带着旧对话继续。

问题发送后先比较聊天内容区域的 ADB 原始 PNG，顶部系统状态栏和底部输入区不参与稳定性判断；内容连续 3 秒无变化且层级不再显示生成状态后进入小荷完成确认。新会话会在当前回答底部使用交替触点持续探测；若 UI 层级严格识别到聊天区下部居中的固定到底按钮，则先由 uiautomator2 直达底部。点击后会重新读取层级，按钮仍存在时最多再受控点击两次，每次都使用最新坐标；连续三次仍未消失便停止点击并交给滚动探测，识别不到按钮时也直接逐屏向下探测。完成确认和返回顶部只负责边界导航，因此使用按实时聊天视口计算的约 78% 长距离快速滑动；正式截图仍保持约 45% 高重叠滚动。两种路径都只有在底部至少连续三次不可继续滚动、画面持续约 6 秒不变且层级没有生成状态后，才允许回到顶部开始正式截图；直达按钮只减少到达底部的滚动次数，不替代完整性验证，也不依赖免责声明、操作栏、问诊卡片或固定文案。该阶段通过 `reply_completion_navigation_method`、`reply_completion_navigation_clicks`、`reply_completion_navigation_target_cleared`、`reply_completion_confirmed_before_capture`、`reply_completion_confirmation_probes`、`reply_completion_confirmation_resets` 和 `reply_completion_confirmation_quiet_ms` 记录路径与证据。完成确认后使用两个交替横向触点持续向上滚动，只有连续两次稳定画面无变化才确认到顶；到顶后对 ADB 原始 PNG 执行 OCR，识别“根据…篇资料为你总结”并按物理截图与逻辑 UI 尺寸映射点击，确认资料展开后才截取第一屏。

小荷正式正文采集在每次滚动结束后读取一次 UI 层级和一张 ADB 原始 PNG，并用相邻视口接缝本身确认画面已经形成一致位移；只有接缝或加载状态存疑时才执行“ADB PNG → UI 层级 → ADB PNG”夹心重采，避免 scrcpy 全屏活动先等待再降级，也避免每个正常视口重复截图。相邻视口按四个分散正文区域的严格多数位移确认接缝；三个区域在按视口比例计算的小容差内一致时，取最小重叠值保留安全重复，2:2 或无多数的位移冲突仍会拒绝无缝裁切。UI 层级未暴露固定下箭头时，图像识别会按竖屏宽度恢复完整外框并从阴影上方截取，避免按钮或圆弧重复进入长图。随后严格单向向下采集，任意内容位移都会清零到底计数，只有连续两次向下滚动无变化才确认到底。接缝仍无法验证时继续保留完整下一视口，允许重复但不裁掉可能的新内容。免责声明、操作栏和参考药品入口都不是必需的结束标记；参考药品仅在实际出现时进入药品终止序列。元数据中的 `reply_top_navigation_method`、`reply_top_confirmed`、`reply_top_confirmation_swipes` 和 `reply_question_structure_validation_required` 记录实际验收方式。推荐药品入口在点击前会等待页面稳定并从最新层级刷新坐标，避免正文重排后点击旧位置；抽屉识别兼容普通窗口和华为弹窗窗口，并使用面板和列表相对竖屏高度确认全屏状态。元数据中的 `reference_products_trigger_refreshed` 记录本题是否刷新过入口坐标。抖音搜索超时会在当前题调试目录保留 `搜索超时.png` 和 `搜索超时.xml` 供本机诊断，这些采集产物仍受忽略规则保护。

引用资料标题最多尝试点击三次：第一次点击 OCR 文字框中心；仍明确折叠时，每次重新 OCR 获取最新位置，第二、第三次分别在文字框中心左右的安全范围内做小幅随机偏移。每次点击后都会确认展开状态，一旦展开立即停止，不能盲目连续点击。

抖音入口使用双路径流程：打开搜索页并搜索问题后，识别“小荷AI医生”智能总结的居中全文控件和独立小程序入口卡片。抖音会把这些卡片文字绘制在画面中但不一定暴露给 UI XML，因此先使用层级结构；层级没有可靠目标时，才对稳定的 ADB 原始 PNG 启用 RapidOCR。OCR 必须同时高置信识别“小荷AI医生”、“根据医学数据智能总结”和位于其下方合理距离的“查看全文”，单独识别到通用按钮不会点击；图片物理坐标会按实时 UI 逻辑尺寸映射，最终仍由 uiautomator2 点击。只有全文控件时保存完整搜索结果页为 `回答_智能总结.png` 并点击全文，同时出现展开型回答和独立小程序卡片时优先走可强校验的独立卡片，避免把抖音自身的“AI生成回答/展开更多”误当成小荷总结。如果层级明确暴露“AI生成回答/展开更多”，脚本也会直接忽略它。如果短暂优先等待后仍未出现智能总结，但识别到独立入口卡片，则保存搜索页为 `回答_小程序入口.png`，按卡片标题区域的相对位置点击，并通过 `MiniAppHostActivity` 和小程序外壳强校验跳转成功。点击前的稳定证据只覆盖小荷卡片本身，旁边无关的自动播放视频不会再阻塞入口；完整搜索页仍由同一时刻的 ADB 原始 PNG 保存。独立入口打开后还会用本地 OCR 要求正文至少包含两个当前问题的有效相邻文字片段；只出现历史会话时等待最多约 15 秒便明确拒绝，不能把任意已有回答当成本题结果。首屏没有两种结果时会有限向下扫描搜索结果；第一轮完成三次扫描仍无结果时，脚本按实际扫描距离回到顶部，确认搜索框仍是当前问题后下拉刷新，并完整执行第二轮识别和扫描。第二轮仍无结果才明确失败、保存搜索超时现场并继续后续任务，不进行第三轮，也不重新输入或重复点击搜索按钮。成功元数据中的 `douyin_search_attempts`、`douyin_search_refreshed` 和 `douyin_search_target_detection_method` 记录实际识别轮次、是否刷新及最终证据来源。

两条抖音路径进入小程序后，都先向下探测到回答底部；只有底部至少连续三次不可继续滚动、正文区域持续约 6 秒无变化且层级没有生成状态，才允许回到顶部开始正式截图。因此不会依赖问诊卡片、推荐问题或固定尾部文案，也不会因一次短暂停顿提前截取。完成确认会记录 `reply_completion_confirmed_before_capture`、探测次数、稳定时长和重置次数。回顶边界只比较上半部正文，避免抖音在真实顶部反复弹出的“没有更多了”Toast重置计数；确认后等待 Toast 消失再保留正式首帧，并设置 90 秒或 80 次滚动的硬上限，无法确认时明确失败而不是无限刷新。返回顶部和正式滚动的正常候选帧各读取一次 UI 层级和一张 ADB 原始 PNG；多区域接缝本身能够证明一致位移时直接使用，只有层级加载或四个分散区域无法形成严格多数时才执行“ADB PNG → UI 层级 → ADB PNG”夹心重采。三个区域在比例容差内一致时取最小重叠，保留安全重复；重采后仍无法证明接缝时保留完整下一视口和浅色分隔，允许重复但不裁掉可能的新内容。成功接缝、滚动决策和异常重采证据都会写入 `接缝汇总.json`。正文根据小程序真实滚动视口排除固定的咨询人选择栏、顶部工具栏和底部输入区；顶部和末端仍分别需要连续两次滚动无变化确认。出现无文字的“参考药品”卡片时，脚本会按右上箭头和商品图的结构识别入口，立即进入药品终止序列：打开抖音 BottomSheet、确认首项图片已加载、持续滚动到真实末项并另存 `回答_参考药品_001.png`。入口、抽屉和商品图片均使用相对竖屏尺寸及节点层级识别，不依赖 1080×2400 固定坐标。JSON 的 `douyin_result_mode` 区分 `smart_summary` 与 `miniapp_entry_card`；入口路径额外记录 `douyin_miniapp_entry_detected`、入口截图及卡片/点击范围、宿主 Activity 和 `douyin_miniapp_entry_opened`，智能总结路径继续记录 `douyin_search_summary_captured`、`douyin_search_summary_screenshot` 和 `douyin_view_full_opened`。正文和药品验收分别记录 `reply_continuity_verified`、`reference_products_detected`、`reference_products_images_ready`、`reference_products_confirmed_end` 和 `reference_products_capture_complete`。

今日头条入口使用对应的独立搜索流程：脚本先按头条首页顶部搜索框的语义节点打开搜索页，再通过搜索页编辑框输入并确认问题，两阶段均不使用固定坐标。头条冷启动时允许最多 30 秒等待其 UI 层级出现，其他入口仍保持 8 秒；若随后出现华为桌面的“添加到主屏幕”提示，只有同时确认弹窗标题、今日头条应用名和可点击“取消”按钮时才会安全关闭，绝不会点击“添加”。首页等待截止后还会执行一次最终层级探测；若此时刚出现搜索入口，只为搜索页编辑框附加最多 3 秒的有限等待。华为系统不提供常见 `mCurrentFocus` 字段时，前台窗口诊断会从 WindowManager 按 Z 轴排列的第一个 Activity 窗口读取，不再反复重启 sidecar。随后优先从 UI 层级等待“小荷AI医生·智能总结”和可点击的全文入口；头条当前可能显示“查看更多”或“查看全文”，两者都必须与品牌标题共同满足位置和置信度约束，不能凭通用按钮文字单独点击。只有结果页搜索框仍与当前原始问题逐字一致时才接受卡片，切换中的旧查询结果会被忽略。头条 WebView 未暴露文字时，才对稳定的 ADB 原始 PNG 启用 RapidOCR 兜底。OCR 返回图片物理坐标，脚本按实时 UI 层级逻辑尺寸显式映射，并在保存稳定搜索结果截图后再次识别确认，随后仍由 uiautomator2 点击且强校验全文页已经打开；若已确认进入小程序宿主但只读层级连接刚好重启，会按宿主就绪条件继续等待，而不会重放点击。若完整等待窗口内确实没有召回目标卡片，脚本只会在重新精确确认搜索框内容后，用同一个原始问题受控重试一次；UI 读取、点击或前台应用异常不会触发重试。完整搜索结果页保存为 `回答_智能总结.png`。进入全文后先持续向下确认真实底部，只有底部至少连续三次不可继续滚动、正文区域持续约 6 秒无变化且层级没有生成状态，才回顶正式截图；正文使用多区域接缝共识，正常帧只采一张 ADB PNG，接缝搜索以本次实际滚动距离为安全下界，同时允许末屏因到达底部只移动很短而产生更大重叠，不再用全范围重搜接受重复卡片形成的伪接缝；接缝不确定时才局部重采，仍无法验证则保留完整下一视口避免漏内容。JSON 额外记录 `toutiao_search_attempts`、`toutiao_search_repeated_exact_question`、`toutiao_answer_card_detection_method`、OCR 引擎/耗时/置信度及物理和逻辑坐标、`toutiao_search_summary_captured`、`toutiao_view_more_opened`、`toutiao_full_page_confirmed_top`、`toutiao_full_page_confirmed_end` 和回答完成确认字段。

头条全文入口打开后还会检查全文目标页：新版正确回答页本身也带“发送消息”输入框，因此该输入框不再单独作为错误路由证据。脚本会同时确认小程序滚动视口，并将 ADB 原始 PNG 的物理尺寸映射到层级逻辑尺寸，只检查视口上部正文区域是否具有真实内容像素；正文已出现时立即接受，连续三次仍为空白才按错误路由处理。若明确进入空白咨询页且本题尚未使用搜索重试额度，脚本会关闭该页、再次精确确认同一个原始问题并仅重试一次；读取或点击异常不会触发该路径。JSON 通过 `toutiao_full_answer_open_attempts` 和 `toutiao_full_answer_route_repeated` 记录全文路由尝试。
...
- 正文采集内部以 `{ frame, transition }` 原子页面序列保存：首帧接缝为 `null`，其余每一帧只能连同上一帧的已验证或安全降级接缝一起加入，因此不会再形成帧数与接缝数不一致的产物。
- 每个普通回答接缝最多局部重采一次；末屏短距离滚动遇到一侧大面积同色区域时，会将各区域候选位移重新交给全部内容区域进行像素校验。接缝失败后会记录多个横向区域的位移一致性、下一屏底部内容特征和层级位移。已经有一次无变化或层级明确显示回答尾部时，仍无法证明新内容的变化会按无进展继续确认，不加入原子页面序列；若换触点只产生区域位移互相矛盾的画面，会明确记录为底部回弹并丢弃。尚未取得任何到底上下文时仍以正文完整性优先，无法验证的下一屏保留为安全降级，不会因证据不足静默丢掉可能的新正文。
- 固定向下箭头优先由 UI 层级识别；层级没有暴露节点时，会在 ADB 原始 PNG 中按相对视口识别中央圆形向下箭头。识别成功后，稳定性比较和正式裁剪都结束于箭头上方，滚动距离随干净区域同步缩短，使箭头下方正文在下一屏重新进入可拼接区域。
