...
小荷 App 在发送前会强制清空并核对输入框文本；截图前必须重新定位到本题问题气泡，否则会明确失败，绝不把旧会话回答记为成功。成功元数据会记录 `reply_question_located=true`。推荐药品抽屉使用面板和列表相对竖屏高度确认全屏状态，兼容不同分辨率与系统状态栏高度。抖音搜索超时会在当前题目录保留 `搜索超时.png` 和 `搜索超时.xml` 供本机诊断，这些采集产物仍受忽略规则保护。
