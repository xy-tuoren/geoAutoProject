...
- scrcpy观察器不解码、不保存也不参与拼接，只根据官方视频包的活动情况判断页面是否停稳；截图区域在滚动后必须等到最后一个活动帧后持续无活动，读取层级和截取PNG期间出现任意活动帧都会作废本次视口，避免把两次生成/重排状态拼进同一张长图。观察器不可用或无法及时确认时会明确记录原因，并改用“ADB截图→uiautomator2层级→ADB截图”双帧夹心校验，不影响UI后端选择。
