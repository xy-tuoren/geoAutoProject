...
test('操作遥测记录成功、失败、耗时和可执行的优化候选', async () => {
  ...
  assert.equal(report.operation_telemetry_version, 2)
  assert.equal(report.recent_operations_ref, '#/operations')
  assert.equal(Object.hasOwn(report, 'recent_operations'), false)
  assert.deepEqual(report.operations, events.map(event => event.details))
