...
const { uniqueQuestions } = require('./question-plan')

async function loadQuestionFile(filePath, questionColumn = '问题') {
  const extension = path.extname(filePath).toLowerCase()
  if (extension === '.txt') {
    return uniqueQuestions((await fs.readFile(filePath, 'utf8')).split(/\r?\n/).filter(line => !line.trim().startsWith('#')))
  }
  if (extension === '.json') {
    ...
    return uniqueQuestions(values.map(item => typeof item === 'object' && item ? item[questionColumn] ?? item.question ?? item['问题'] : item))
  }
  if (extension === '.csv' || extension === '.xlsx' || extension === '.xlsm') {
    ...
    return uniqueQuestions(rows.map(row => row[questionColumn]))
  }
  throw new Error('仅支持 TXT、CSV、JSON、XLSX 和 XLSM 问题文件。')
}

module.exports = { loadQuestionFile }
